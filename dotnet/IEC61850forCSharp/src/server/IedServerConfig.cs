...
namespace IEC61850
{
    ...
    namespace Server
    {
        ...
        public class IedServerConfig : IDisposable
        {
            ...
            /// <summary>
            /// Releases all resource used by the <see cref="IedServerConfig"/> object.
            /// </summary>
            /// <remarks>Call <see cref="Dispose"/> when you are finished using the <see cref="IedServerConfig"/>. The
            /// <see cref="Dispose"/> method leaves the <see cref="IedServerConfig"/> in an unusable state. After
            /// calling <see cref="Dispose"/>, you must release all references to the
            /// <see cref="IedServerConfig"/> so the garbage collector can reclaim the memory that the
            /// <see cref="IedServerConfig"/> was occupying.</remarks>
            public void Dispose()
            {
                lock (this)
                {
